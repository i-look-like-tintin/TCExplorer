class VisualizationRenderer {
    ...
    drawComparisonTrack(cyclone, scenarioType, useIntensity = false) {
        ...
        if (useIntensity) {
            ...
            // Find genesis point index (first point ≥34 knots) and start track from there
            ...
            if (trackFromGenesis.length < 2 && cyclone.track.length >= 2) {
                ...
                console.warn(`${cyclone.name}: Using full track (genesis filter resulted in ${trackFromGenesis.length} points)`);
            }
            
            // Debug logging
            console.log(`${cyclone.name}: Original=${cyclone.track.length}, Genesis@${genesisIndex}, Filtered=${trackFromGenesis.length}`);
            
            if (trackFromGenesis.length < 2) {
                console.warn(`${cyclone.name}: Track too short for rendering (${trackFromGenesis.length} points)`);
                return;
            }
            
            const latlngs = trackFromGenesis.map(point => [point.lat, point.lon]);
    ...
    drawComparisonIntensityTrack(cyclone, scenarioType) {
        ...
        if (trackFromGenesis.length < 2) return;
        
        const baseColor = this.comparisonColors[scenarioType].track;
        
        for (let i = 0; i < trackFromGenesis.length - 1; i++) {
            const start = trackFromGenesis[i];
            const end = trackFromGenesis[i + 1];
    ...
    drawComparisonGenesisPoint(cyclone, scenarioType) {
        ...
        // Find first point where wind reaches 34 knots (63 km/h)
        ...
        marker.bindPopup(`
            ...
            Year: ${cyclone.year}<br>
            Coordinates: ${genesis.lat.toFixed(2)}°, ${genesis.lon.toFixed(2)}°<br>
            Wind Speed: ${genesis.windSpeed} km/h
        `);
    ...
    drawGenesisPoint(cyclone) {
        ...
        marker.bindPopup(`<strong>${cyclone.name}</strong><br>Genesis: ${genesis.date}<br>Year: ${cyclone.year}<br>Coordinates: ${genesis.lat.toFixed(2)}°, ${genesis.lon.toFixed(2)}°<br>Wind Speed: ${genesis.windSpeed} km/h`);
